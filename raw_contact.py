...
SeqInfo = namedtuple('SeqInfo', ['localid', 'refid', 'name', 'length', 'sites','GC' , 'cov']) #create a new class of tuple: SeqInfo
...
class ContactMatrix:
    ...
    def __init__(self, bam_file, enzymes, seq_file, coverage_file , path , min_mapq=30, min_len=1000, min_match=30, min_signal=2 ):
        ...
        #valid_contact is the dictionary storing the valid contacts#
        ...
        ##test that BAM file is the correct sort order
            ...
            # minimum length threshold
                ...
                self.seq_info.append(SeqInfo(localid , n , rname, rlen, fa['sites'] , fa['GC'] , cov_temp))
                localid = localid + 1
                offset += rlen
        ...
        for i , idn in enumerate(contig_id):
            ...
            seq_temp.append(SeqInfo(i , seq.refid , seq.name , seq.length , seq.sites , seq.GC , seq.cov))
        ...
        logger.info('Writing valid contacts...')
    ...
    def _write_must_contact(self):
        self.seq_map = self.seq_map.tocoo()
        _row = self.seq_map.row
        _col = self.seq_map.col
        _data = self.seq_map.data
        _index = _row < _col
        _row = _row[_index]
        _col = _col[_index]
        _data = _data[_index]
        self.valid_contact = list(zip(_row,_col,_data))
